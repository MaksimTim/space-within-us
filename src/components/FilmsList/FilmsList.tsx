  ...
  return (
    <div className="flex flex-row p-2 my-5 border-2 rounded-md shadow-md">
      <div className="basis-1/4">
        <img src={item.posterUrlPreview} width={"200"} />
      </div>
      <div className="basis-1/2 px-3">
        ...
        <div className="pt-6">
          ...
          {item.countries.map((c) => (
            <span className="text-blue-800" key={c.country}>
              {c.country}{" "}
            </span>
          ))}
        </div>
        <div>
          Жанр:{" "}
          {item.genres.map((g) => (
            <span className="text-blue-800" key={g.genre}>
              {g.genre}
            </span>
          ))}
        </div>
      ...
      <div className="basis-1/4 text-center text-2xl border-2 rounded-md">
        {item.ratingKinopoisk && (
          <div className="pt-2">
            Kinopoisk:{" "}
            <span className="bg-orange-400 p-1">{item.ratingKinopoisk}</span>
          </div>
        )}
        {item.ratingImdb && (
          <div className="pt-3">
            IMDb: <span className="bg-green-400 p-1">{item.ratingImdb}</span>
          </div>
        )}
      </div>
    </div>
